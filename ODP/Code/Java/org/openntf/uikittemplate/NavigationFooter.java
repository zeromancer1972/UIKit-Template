...
import org.openntf.uikittemplate.Page;
...
public class NavigationFooter implements Serializable {
	...
	public NavigationFooter() {
		...
		this.navigation.add(new Page("Subpage", "#", "", false));

		try {
			admin = ExtLibUtil.getCurrentDatabase().queryAccessRoles(
					ExtLibUtil.getCurrentSession().getEffectiveUserName()).contains("[Admin]");
			// render if not logged in
			navigation.add(new Page("Login", "login.xsp", "", false, ExtLibUtil.getCurrentSession()
					.getEffectiveUserName().equals("Anonymous")));
